...
describe('Exercise Service Test', () => {
    ...
    beforeAll(async () => {
        const loginResponse  = await new UsersController().login({id: "test", password: "test"})
        authorizedToken = loginResponse.authorizedToken
    })

    it('게시물 등록 테스트', async () => {
        expect(await ExerciseService().addExercisePost({
            authorizationToken:authorizedToken, exercisePostRequest: {
                contents: "test", exerciseType: ExerciseType.SQUARS, photoUrls: ['test', 'test1'], title: "test"
            }
        })).toBeTruthy()
    })
    ...
    it('게시물 등록시 인증 없이 시도할 경우', async () => {
        await expect(async () => {
            await ExerciseService().addExercisePost({
                authorizationToken:undefined, exercisePostRequest:{
                    contents: "test", exerciseType: ExerciseType.SQUARS, photoUrls: ['test', 'test1'], title: "test"
                }});
    ...
    it('게시물 등록시 제목에 데이터를 넣지 않고 시도할 경우', async () => {
        await expect(async () => {
            await ExerciseService().addExercisePost({
                authorizationToken:authorizedToken, exercisePostRequest:{
                    contents: "test", exerciseType: ExerciseType.SQUARS, photoUrls: ['test', 'test1'], title: ""
                }});
